...
// Bounded best-effort flush of frozen forensic evidence for terminal exit
// paths that never reach normal session teardown (window close, emergency
// quit).  Reuses the normal bounded teardown drain (quiet/deadline capped),
// so worst case adds roughly two seconds before exit and only when a capture
// or latched evidence exists; otherwise a cheap no-op.  Gated by
// [Others] ExperimentalEmergencyEvidenceFlush (default on).  Must NOT be
// called from DllMain/loader-lock or crash-handler contexts.
