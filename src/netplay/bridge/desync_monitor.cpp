...
// Ring depths.  The region ring must comfortably hold the confirmation
// window (kConfirmMismatchFrames) plus the 10+ frames before the divergence
// the dump wants to show.
constexpr size_t kRegionRingDepth = 128;
constexpr size_t kSampleRingDepth = 512;
constexpr size_t kCompareLogDepth = 64;

// A desync is declared after this many consecutive distinct mismatching
// frames with no matching frame in between.  Rollback transients re-match
// within a few frames; real desyncs never re-match.
constexpr int kConfirmMismatchFrames = 32;
...
constexpr int kSendEverySamples = 4;
...
// Compare a (frame, remoteSum) pair against the local ring. Caller holds the
// lock. Returns true when the desync just became confirmed.
bool CompareRemoteSampleLocked(int frame, uint32_t remoteSum)
{
...
void WriteDesyncDump()
{
    ...
    if (report != INVALID_HANDLE_VALUE)
    {
        ...
        for (size_t i = 0; i < kCompareLogDepth; ++i)
        {
            ...
            AppendFormat(
                report,
                "  frame=%d local=0x%08lX remote=0x%08lX %s\r\n",
                e.frame,
